...
import io.sherpair.w4s.auth.domain.{Kind, Member, Token, UniqueViolation}
import tsec.common.SecureRandomId

private[doobie] class TokenSql[F[_]: Sync] {

  implicit val kindMeta: Meta[Kind] = pgEnumStringOpt("kind", Kind.withNameOption, _.entryName)
  ...
  def deleteIfOlderThanSql(rateLimit: FiniteDuration, member: Member, kind: Kind): Update0 =
    sql"""
      DELETE FROM tokens
  ...
  def findSql(tokenId: SecureRandomId, kind: Kind): Query0[Token] =
    sql"""SELECT * FROM tokens WHERE token_id = $tokenId AND kind = $kind""".query[Token]
  ...
  def insertStmt(token: Token): Update0 =
    sql"""
      INSERT INTO tokens (token_id, member_id, kind, expiry_date)
      VALUES (${token.tokenId}, ${token.memberId}, ${token.kind}, ${token.expiryDate})
    """
    .update
