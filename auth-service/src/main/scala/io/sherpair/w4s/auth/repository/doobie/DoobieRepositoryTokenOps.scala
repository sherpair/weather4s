...
private[doobie] class DoobieRepositoryTokenOps[F[_]: Sync](
  ...
  override def deleteIfOlderThan(rateLimit: FiniteDuration, user: User): F[Boolean] =
    deleteIfOlderThanSql(rateLimit, user).run.map[Boolean](_ > 0).transact(tx)
  ...
  override def insert(token: Token): F[Token] = insertSql(token).transact(tx).as(token)
