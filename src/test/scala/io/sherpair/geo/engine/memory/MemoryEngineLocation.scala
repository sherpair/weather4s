...
import io.sherpair.geo.domain.{BulkError, GeoError, GeoPoint, Location}
import org.apache.lucene.document.{Document, LatLonDocValuesField, LatLonPoint, StoredField}
import org.apache.lucene.index.IndexableField
import org.apache.lucene.search.SortField
import org.apache.lucene.search.SortField.Type

class MemoryEngineLocation[F[_]: Sync](ref: Ref[F, SearchableLocation]) {
  ...
  def upsert(location: Location): F[String] =
    Sync[F].raiseError(GeoError(s"Bug in LocationEngineIndex: upsert should never be called"))
}
...
object MemoryEngineLocation {
  def apply[F[_]: Sync]: F[MemoryEngineLocation[F]] = {
